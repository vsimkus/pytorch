...
const std::vector<std::string> functions = {
    ...
    R"(
        ...
        def clamp_with_tensors_0(self,
            ...
            def backward(grad_output):
                if min is not None and max is not None:
                    ge_min = self >= min.type_as(self)
                    self_mask = ge_min * (self <= max.type_as(self))
                    min_mask = self < min.type_as(self)
                    max_mask = ge_min * (self > max.type_as(self))
        ...
        def clamp_with_tensors_1(self,
                  ...
                  max: number):
            def backward(grad_output):
                self_mask = ((self >= min.type_as(self)) * (self <= float(max))).type_as(self)
                min_mask = self < min.type_as(self)
                return grad_output * self_mask, grad_output * min_mask, None
            return torch.clamp_with_tensors(self, min=min, max=max), backward
        ...
        def clamp_with_tensors_2(self,
                  ...
                  max: Tensor):
            def backward(grad_output):
                self_mask = ((self >= float(min)) * (self <= max.type_as(self))).type_as(self)
                max_mask = ((self >= float(min)) * (self > max.type_as(self))).type_as(self)
                return grad_output * self_mask, None, grad_output * max_mask
            return torch.clamp_with_tensors(self, min=min, max=max), backward
    )"};
